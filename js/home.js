...
$.post("https://www.chauffeursvtc.com/appclient/open_login_app.php", { tel: tel, mngid: mngid, log: tel, pass: pwd, dep: dep, version: appVersion}, function(data) {
	/*
	if(data.done) {
	...
	//else alert('Pas de correspondance dans la table opendata_interface !!', alertDismissed, 'ChauffeursVTC Erreur', 'OK');
	...
	*/
	...
	/*
	if(data.done) {
		$.post("https://www.chauffeursvtc.com/appclient/open_enroll_app.php", { tel: tel, insee: insee, dep: dep, mngid: mngid, ads: ads, cpro: cpro, imat: imat}, function(data) {
			if(data.taxi_id!='') {
				openDataInit=true;
				openDataGo=true;
			}
...
$( '#directions_map' ).live( 'pagebeforeshow',function(event){
	...
	if (rdv != '')
	{
		$.post("https://www.chauffeursvtc.com/appclient/in_app_calls.php", { map: 'true', cmd: cmd, rdv: rdv, com: com, idcourse: idcourse, cell: cell, pass: pass, dep: dep }, function(data){
			...
			self.set('getCurrentPosition', function() {
				...
				self.getCurrentPosition( function(position, status) {
					if ( status === 'OK' ) {
						...
						navigator.notification.alert('Unable to get current position', alertDismissed, 'ChauffeursVTC Erreur', 'OK');
					}
...
$( '#planning' ).live( 'pagebeforeshow',function(event){
	$.mobile.loading( "show" );
	$.post("https://www.chauffeursvtc.com/appclient/in_app_calls.php", { planning: 'true', tel: tel, pass: pass, dep: dep, mngid: mngid }, function(data){
...
$( '#history' ).live( 'pagebeforeshow',function(event){
	$.mobile.loading( "show" );
	$.post("https://www.chauffeursvtc.com/appclient/in_app_calls.php", { history: 'true', tel: tel, pass: pass, dep: dep, mngid: mngid }, function(data){
		if (data != 0)
		{
...
$( '#infos' ).live( 'pagebeforeshow',function(event){
	$.mobile.loading( "show" );
	$.post("https://www.chauffeursvtc.com/appclient/in_app_calls.php", { infos: 'true', pass: pass, dep: dep }, function(data){
...
function getLocation()
{
	if (navigator.geolocation)
	{
		...
		if(app) navigator.notification.alert("Localisation impossible, veuillez v&eacute;rifier l'&eacute;tat de votre connection ainsi que la disponibilit&eacute; des services de localisation dans les réglages de votre appareil.", alertDismissed, 'ChauffeursVTC', 'OK');
	...
	if (error.code == error.TIMEOUT) {
		...
		navigator.geolocation.getCurrentPosition(get_coords, function(){
			...
			if(!geoFailedAlertOnce) {
				...
				if(app) navigator.notification.alert(geoAlert, alertDismissed, 'ChauffeursVTC', 'OK');
				else alert(geoAlert);
			}
...
function get_coords(position) 
{
	...
	*/
	...
	if((lat!=previousLat) && (lng!=previousLng)) {
		$.post("https://www.chauffeursvtc.com/appclient/insert_app_cab_geoloc.php?lat="+lat+"&lng="+lng, { tel: tel, pass: pass, dep: dep }, function(data) {
			//alert('Sent:'+lat+' , '+lng);
		});
	}
	previousLat = lat;
...
function update()
{
	...
	if(dispo == 1) {
		$.ajax({
			type: "POST",
			url: "https://www.chauffeursvtc.com/appserver/open_get_app_drive_lp.php",
			...
			if (data.gotSome>0)
			{
				...
				if(notifyOnce) {
					...
					cordova.plugins.notification.local.schedule({
						id: 1,
						title: "Notification de course ChauffeursVTC",
			...
			else
			{
				...
				if (gotSome) {
					...
					cordova.plugins.notification.local.schedule({
						id: 1,
						title: "Vous avez manqué une course ChauffeursVTC",
...
function Dispo_On()
{
	$.post("https://www.chauffeursvtc.com/appclient/open_dispo_app.php?dispo=1", { taxi: taxi, tel: tel, pass: pass, dep: dep, taxi_id: taxi_id, opendata: openDataGo }).done(function(data) {
...
function Dispo_Off()
{
	$.post("https://www.chauffeursvtc.com/appclient/open_dispo_app.php?dispo=0", { taxi: taxi, tel: tel, pass: pass, dep: dep, taxi_id: taxi_id, opendata: openDataGo }).done(function(data) {
...
function onComing()
{
	...
	$.post("https://www.chauffeursvtc.com/appclient/open_dispo_app.php?dispo=0", { taxi: taxi, tel: tel, pass: pass, dep: dep, taxi_id: taxi_id }).done(function(data) {
		$("#dispo").empty().append('<a href="#home" onClick="Dispo_On()"><img src="visuels/DispoOff_flat.png" width="100%"/></a>');
		$("#dispo_jobs").empty().append('<a href="#jobs_taker" onClick="Dispo_On()"><img src="visuels/DispoOff_flat.png" width="100%"/></a>');
		$("#dispo_cmd").empty().append('<a href="#cmd" onClick="Dispo_On()"><img src="visuels/DispoOff_flat.png" width="100%"/></a>');
		$.sessionStorage.setItem('dispo', '0');
...
function footer()
{
	$.post("https://www.chauffeursvtc.com/appclient/footer_app.php", { dep: dep, no_marquee: true }, function(data) {
		for (i=0; i<9; i++) {
...
function addCalendar(date, rdv, com, idcourse, cell)
{
	...
	var success = function(message) { navigator.notification.alert("EVENEMENT AJOUTE AU CALENDRIER", alertDismissed, 'ChauffeursVTC', 'OK'); };
	var error = function(message) { navigator.notification.alert("Erreur: " + message, alertDismissed, 'ChauffeursVTC Erreur', 'OK'); };
	// create
...
{
	$.post("https://www.chauffeursvtc.com/appclient/justify.php", { when: when, rdv: rdv, comments: comments, destadd: destadd, cell: cell, dep: dep, pass: pass, email: email }, function(data){
		$.mobile.loading( "show" );
		navigator.notification.alert(data, alertDismissed, 'ChauffeursVTC', 'OK');
		//window.plugins.childBrowser.showWebPage('https://www.chauffeursvtc.com', { showLocationBar: true });
	}).always(function() { $.mobile.loading( "hide" ); });
}
function reporting_customer(rdv_rc, idcourse_rc, hail_id_rc, operator_rc, cell_rc)
{
	var comRate = $("#leComRate").val();
	$.post("https://www.chauffeursvtc.com/appclient/open_reporting.php", { rdv: rdv_rc, idcourse: idcourse_rc, hail_id: hail_id_rc, operator: operator_rc, cell: cell_rc, dep: dep, tel: tel, mngid: mngid, comments: comRate}, function(data){ 
		if(app) navigator.notification.alert('Votre remarque a bien été prise en compte, Merci.', alertDismissed, 'ChauffeursVTC', 'OK');
...
function cancelCall(query_string)
{
	$.mobile.loading( "show" );
	dep = $.localStorage.getItem('dep');
	$.post("https://www.chauffeursvtc.com/appserver/open_diary_app_dcvp.php?dep="+dep, query_string, function(data){ 
...
function diaryCall(query_string)
{
	$.mobile.loading( "show" );
	dep = $.localStorage.getItem('dep');
	$.post("https://www.chauffeursvtc.com/appserver/bookings_app_dcvp.php?dep="+dep, query_string, function(data){ 
		switch (data.location) {
			 case '#directions_map':
				//navigator.notification.alert('in direction case');
				$.sessionStorage.setItem('rdv', data.rdv);
...
{
	$.post("https://www.chauffeursvtc.com/appserver/open_status.php?dep=" + d + "&check=0" , q, function(data){ 
		if (data != 0)
		{
			stillCheckingHail = false;
			if(app) navigator.notification.alert(data, alertDismissed, 'ChauffeursVTC', 'OK');
			else alert(data);
			$.mobile.pageContainer.pagecontainer("change", "#home", { transition: "slide"} );
			cordova.plugins.notification.local.schedule({
				id: 1,
...
function callIncident(irdv, ihail, iop, icell, istatus)
{
	...
	$.post("https://www.chauffeursvtc.com/appserver/open_incident_reason.php" , { rdvpoint: irdv, hail_id: ihail, operator: iop, cell: icell, status: istatus, incident_taxi_reason: incident_taxi_reason, db: 'true', dep: dep}, function(data){ 
		if (data.ok)
		{
			var number = icell;
			...
			if(app) navigator.notification.alert("Erreur: L'incident n'a pas été déclaré.", alertDismissed, 'ChauffeursVTC', 'OK');
...
function getLocationOnce()
{
	if (navigator.geolocation)
	{
		...
		navigator.notification.alert("Localisation impossible.", alertDismissed, 'ChauffeursVTC Erreur', 'OK');
	}
}
...
{			
	...
	$.post("https://www.chauffeursvtc.com/appclient/secure_xml.php", { lat: lat, lng: lng, dep: dep, pass: pass}, function(xml){
		...
		$(xml).find('marker').each(function(){
			...
			$.post("https://www.chauffeursvtc.com/appclient/secure.php", { taxi: name, tel: address, rdvpoint: rdvpoint, helptaxi: taxi, helpname: helpname, helptel: tel, idcourse: idcourseUrg, num_req: num_reqUrg, dep: dep, pass: pass}, function(data){
...
function check_answer()
{
	...
	sec = setInterval( function () {
		$.post("https://www.chauffeursvtc.com/appclient/status.php?idcourse=" + idcourseUrg + "&check=1" , { dep: dep}, function(data){ 
			if (data != 0)
			{
...
{
	idcourseUrg = $.sessionStorage.getItem('idcourseUrg');
	$.post("https://www.chauffeursvtc.com/appclient/secure.php", { taxi: '', tel: '', rdvpoint: '', helptaxi: taxi, helpname: '', helptel: tel, idcourse: idcourseUrg, dep: dep, pass: pass, stopcall: 'true'}, function(data){
		$.mobile.pageContainer.pagecontainer("change", "#home", { transition: "slide"} );
	});
...
{
	//window.plugins.childBrowser.showWebPage('https://www.chauffeursvtc.com/redir.php', { showLocationBar: true });
...
if ( app ) {
	...
	function onDeviceReady() {
		...
		$.post("https://www.chauffeursvtc.com/appclient/polling.php", {version: appVersion, os: devicePlatform}, function(data) {
			...
			if(data.pop=='OK') { // App update here for iOS devices...
				openSomeUrl('https://www.chauffeursvtc.com/updates/');
				//openSomeUrl('itms-services://?action=download-manifest&amp;url=https://www.chauffeursvtc.com/updates/'+data.filename);
			}
		}, "json").always(function(data) {
		...
		*/
		BackgroundGeolocation.on('error', function(error) {
			//if(isApp) navigator.notification.alert('BackgroundGeolocation error', App.alertDismissed, 'ChauffeursVTC.com', 'OK');
			//else alert('BackgroundGeolocation error');
			navigator.notification.confirm('Erreur de Géolocalisation, voulez-vous aller dans les réglages afin d\'activer le service de géolocalisation pour cette app ?', 'ChauffeursVTC.com', function() {
		...
		AppRate.preferences = {
			...
			displayAppName: 'ChauffeursVTC.com Chauffeur',
			usesUntilPrompt: 6,
			...
			customLocale: {
				title: "Notez ChauffeursVTC.com Chauffeur",
				message: "Si vous aimez utiliser ChauffeursVTC.com Chauffeur, n’oubliez pas de voter sur l’App Store. Cela ne prend qu’une minute. Merci d’avance pour votre soutien !",
...
function onResume() {
	setTimeout(function() {
		$.post("https://www.chauffeursvtc.com/client/active_app.php", { tel: tel, mngid: mngid, dep: dep}, function(data) {});
...
var scanSuccess = function (result) {
	...
	if (textFormats.match(result.format)) {                
		...
		//setTimeout(function() { window.plugins.childBrowser.showWebPage(searchUrl, { showLocationBar: true }); }, 500);
			  ...
			  " NON SUPPORTE. Valeur du scan: " + result.text, alertDismissed, 'ChauffeursVTC Erreur', 'OK');
	}
}
function goScan ()
{
	cordova.plugins.barcodeScanner.scan(
		scanSuccess, 
		function (error) {
			navigator.notification.alert("Scan Erreur: " + error, alertDismissed, 'ChauffeursVTC Erreur', 'OK');
		}
	);
}
var renameUpload = '';
...
function UDPTransmitterInitializationSuccess(success) {
	//navigator.notification.alert('UDP INIT SUCCESS: '+success, alertDismissed, 'ChauffeursVTC', 'OK');
}
function UDPTransmitterInitializationError(error) {
	navigator.notification.alert('UDP INIT ERROR: '+error, alertDismissed, 'ChauffeursVTC Erreur', 'OK');
}
*/
function myTaxiDown()
{
	//window.open(url,'_blank','location=yes,enableViewportScale=yes,closebuttoncaption=Fermer');
	if(app) window.open('chauffeursvtcclient://?from=driver', '_system');
	else window.open('https://www.chauffeursvtc.com/apps/client/', '_blank');
}
function Share()
{
	var number = $('#telShare').val();
	var message = "Téléchargez l'app ChauffeursVTC en suivant ce lien : https://www.chauffeursvtc.com/stores.php?app=cvclient";
	var intent = ""; //leave empty for sending sms using default intent
	var success = function () {
...
{
	var number = $('#telShare').val();
	var message = "Téléchargez l'app ChauffeursVTC Chauffeur en suivant ce lien : https://www.chauffeursvtc.com/stores.php?app=cvdriver";
	var intent = ""; //leave empty for sending sms using default intent
	var success = function () {
		//navigator.notification.alert('Message sent successfully');
		$('#smsReturn').empty().append('Message envoy&eacute; avec succ&egrave;s, Merci');
...
{
	var number = $('#telShare').val();
	var message = "Téléchargez l'app ChauffeursVTC Business sur les sores en suivant ce lien : https://www.chauffeursvtc.com/stores.php?app=pro  ou rendez-vous sur le WebService en suivant ce lien :  https://www.chauffeursvtc.com/pro/";
	var intent = ""; //leave empty for sending sms using default intent
	var success = function () {
		//navigator.notification.alert('Message sent successfully');
		$('#smsReturn').empty().append('Message envoy&eacute; avec succ&egrave;s, Merci');
...
function modPay() {
	...
	$.mobile.loading( "show" );
	$.post('https://www.chauffeursvtc.com/payzen/updateIndent.php', {cardNumber: cardNumber, exp: exp, cardNetwork: cardNetwork, cvv: cvv, civil: civil, nom: nom, prenom: prenom, tel: tel, email: email, cardIdent: siret, station: station}, function(data){
	}, "json").done(function(data) { 
		var display = '';
		if (data.sniffed == 'OK')
		{
...
$(document).on( 'pagecreate', function() {
	if(pageFirstCreation) { // Avoid this event to trigger those function any time we switch pages
		...
		if(!app) {
			getLocation();
			$.post("https://www.chauffeursvtc.com/appclient/polling.php", {}, function(data) {
				pollingTime = data.polling;
				getBackPollingTime = data.polling;
			}, "json").always(function(data) {
				setTimeout('update()', 2000);
		...
		$('#openSwitch').change(function(){
			if ($(this).val()==1) {
				...
				openDataGo=false;
				$.post("https://www.chauffeursvtc.com/appclient/open_dispo_app.php?dispo=0", { taxi: taxi, tel: tel, pass: pass, dep: dep, taxi_id: taxi_id, opendata: true, getout: true }); 
			}
		});
		*/
	...
	$("#modmy").validate({
		...
		*/
		...
		submitHandler: function (form) {
			...
			$.mobile.loading( "show" );
			// Subs some data
			$.post("https://www.chauffeursvtc.com/appclient/login_app.php", $("#modmy").serialize(), function(data) {
				// GET SHIT BACK !!
				...
				navigator.notification.alert(alertMe, alertDismissed, 'ChauffeursVTC', 'OK');
			});
		}
	});
	/*
	$("#letaxiForm").validate({
		...
		submitHandler: function (form) {
			...
			if(imatCheck.length>6) {
				// Subs some data
				$.post("https://www.chauffeursvtc.com/appserver/open_register.php", dataLeTaxi, function(data) {
					// GET SHIT BACK !!
					$.localStorage.setItem('insee', data.insee);
					$.localStorage.setItem('imat', data.imat);
					$.localStorage.setItem('constructor', data.constructor);
					...
					if (data.ok)
					{
						...
						$.post("https://www.chauffeursvtc.com/appclient/open_enroll_app.php", { tel: tel, insee: insee, dep: dep, mngid: mngid, ads: taxi, cpro: cpro, imat: imat}, function(data) {
							taxi_id = data.taxi_id;
							$.localStorage.setItem('taxi_id', data.taxi_id);
							openStatus = data.status;
							openDataInit=true;
				...
				navigator.notification.alert("L'immatriculation fournie ne semble pas valide (AB123YZ ou AB-123-YZ), veuillez corriger SVP.", alertDismissed, 'ChauffeursVTC', 'OK');
			}
		}
	});
	*/
	$("#change").submit(function(event) {
		...
		// Subs some data
		$.post("https://www.chauffeursvtc.com/appclient/login_app.php", $("#change").serialize(), function(data) {
			// GET SHIT BACK !!
			var display = '';
			var alertMe = '';
			if (data.changed)
