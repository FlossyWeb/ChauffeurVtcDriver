...
function active()
{
	var posting = $.post("https://www.chauffeursvtc.com/appclient/active_app.php", { tel: tel, mngid: mngid, dep: dep}, function(data) {
		actived = data.active;
		// GET SHIT BACK !!
...
function secureCall(position)
{
	...
	$.post("https://www.chauffeursvtc.com/appclient/secure_xml.php", { lat: lat, lng: lng, dep: dep, pass: 'true'}, function(xml){
		...
		$(xml).find('marker').each(function(){
			...
			$.post("https://www.chauffeursvtc.com/appclient/secure.php", { taxi: name, tel: address, rdvpoint: rdvpoint, helptaxi: taxi, helpname: helpname, helptel: tel, idcourse: idcourseUrg, num_req: num_reqUrg, dep: dep, pass: 'true'}, function(data){
...
{
	var idcourseUrg = $.sessionStorage.getItem('idcourseUrg');
	$.post("https://www.chauffeursvtc.com/appclient/secure.php", { taxi: '', tel: '', rdvpoint: '', helptaxi: taxi, helpname: '', helptel: tel, idcourse: idcourseUrg, dep: dep, pass: 'true', stopcall: 'true'}, function(data){
...
function footer()
{
	$.post("https://www.chauffeursvtc.com/appclient/footer_app.php", { dep: dep }, function(data) {
...
{
	//window.plugins.childBrowser.showWebPage('http://www.chauffeursvtc.com/redir.php', { showLocationBar: true });
...
$(document).on( 'pagecreate', function() {
	...
	$("#login").submit(function(event) {
		...
		// Subs some data 
		$.post("https://www.chauffeursvtc.com/appclient/login_app.php", $("#login").serialize(), function(data) {
			// GET SHIT BACK !!
			$.localStorage.setItem('civil', data.civil);
			$.localStorage.setItem('nom', data.nom);
			$.localStorage.setItem('prenom', data.prenom);
	...
	$("#loginCb").submit(function(event) {
		...
		event.preventDefault();
		// Subs some data 
		$.post("https://www.chauffeursvtc.com/appclient/login_app.php", $("#loginCb").serialize(), function(data) {
			// GET SHIT BACK !!
			$.localStorage.setItem('civil', data.civil);
			$.localStorage.setItem('nom', data.nom);
			$.localStorage.setItem('prenom', data.prenom);
...
$(document).ready(function(){
	...
	$("#RegNameStep").validate({
		...
		submitHandler: function (form) {
			...
			$.mobile.loading( "show" );
			// Subs some data
			$.post("https://www.chauffeursvtc.com/appclient/register_app_name.php", $("#RegNameStep").serialize(), function(data) {
				// GET SHIT BACK !!
				$.localStorage.setItem('civil', data.civil);
				$.localStorage.setItem('nom', data.nom);
				$.localStorage.setItem('prenom', data.prenom);
	...
	$("#RegCabStep").validate({
		...
		submitHandler: function (form) {
			...
			$.mobile.loading( "show" );
			// Subs some data
			$.post("https://www.chauffeursvtc.com/appclient/register_app_cab.php", $("#RegCabStep").serialize(), function(data) {
				// GET SHIT BACK !!
				$.localStorage.setItem('taxi', data.taxi);
				$.localStorage.setItem('cpro', data.cpro);
				//$.localStorage.setItem('tel', data.tel);
		...
		submitHandler: function (form) {
			...
			$.post("https://www.chauffeursvtc.com/appclient/register_app_cb.php", $("#RegCbStep").serialize(), function(data) {
				...
				if (data.subscribed && data.payzen)
				{
					...
					var pwd = data.pwd;
					var dep = data.dep;
					$.post("https://www.chauffeursvtc.com/appclient/login_app.php", { log: log, pass: pwd, dep: dep}, function(data) {
						// GET SHIT BACK !!
						$.localStorage.setItem('civil', data.civil);
						$.localStorage.setItem('nom', data.nom);
						$.localStorage.setItem('prenom', data.prenom);
	...
	$("#forget").validate({
		...
		,
		submitHandler: function (form) {
			$.post("https://www.chauffeursvtc.com/appclient/forget_app.php", $("#forget").serialize(), function(data) {
